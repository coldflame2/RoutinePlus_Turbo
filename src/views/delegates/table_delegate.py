...
class TableDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        # first and last row
        if index.row() == 0:
            padded_rect = self.add_padding(option.rect, 10, 3, 0, 2)
            painter.fillRect(padded_rect, QColor(first_and_last))  # Fill with color
        elif index.row() == total_rows - 1:
            padded_rect = self.add_padding(option.rect, 10, 2, 0, 3)
            painter.fillRect(padded_rect, QColor(first_and_last))  # Fill with color
        else:
            padded_rect = self.add_padding(option.rect, 10, 1, 0, 1)
        ...
        padded_rect = self.add_padding(option.rect, 20, 15, 0, 15)
        ...
        if option.state & QStyle.StateFlag.State_MouseOver:
            ...
            padded_rect = self.add_padding(option.rect, 10, 0, 0, 0)
